...
void Building::spawnPerson(Person newPerson)
{
    ...
    (newPerson, newPerson.getTargetFloor() - newPerson.getCurrentFloor());
...
void Building::update(Move move)
{
    ...
    {
        int newArr[MAX_PEOPLE_PER_FLOOR];
        for (int i = 0; i < MAX_PEOPLE_PER_FLOOR; ++i)
        {
            newArr[i] = 0;
        }
        move.copyListOfPeopleToPickup(newArr);

        //call remove people to correct floor
        floors[elevators[move.getElevatorId()].getCurrentFloor()]
        .removePeople(newArr, move.getNumPeopleToPickup());
    }
    //sending elevator to target floor
...
int Building::tick(Move move)
{
    ...
    {
        exploded = exploded + floors[i].tick(time);
    }
    return exploded;
}
